...
## Training
```bash
python super_resolution/train.py
```
## Inference via scripts
```bash
python super_resolution/predict.py
```

## Endpoints
```bash
uvicorn super_resolution.app:app --host 0.0.0.0 --port 5000 --reload
...
1. Build image
```bash
docker build -t mwml-torch-super-resolution:latest -f Dockerfile .
```
2. Run container
```bash
docker run -d -p 5000:5000 -p 6006:6006 --name mwml-torch-super-resolution mwml-torch-super-resolution:latest
```

## Directory structure
```
mwml-torch-super-resolution/
├── datasets/                           - datasets
...
```
python mwml-torch-super-resolution/train.py --overfit
```
...
```
docker run -p 5000:5000 --name mwml-torch-super-resolution mwml-torch-super-resolution:latest
```

• Get inside container if using `CMD ["/bin/bash"]`
```
docker run -p 5000:5000 -it mwml-torch-super-resolution /bin/bash
```
